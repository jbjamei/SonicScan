...
export const AnalysisCard: React.FC<AnalysisCardProps> = ({ analysis, isLoading, onAnalyze, onFeedback, hasAudio }) => {
  return (
    <div className="relative p-[1px] rounded-2xl bg-gradient-to-b from-white/20 to-white/5 overflow-hidden group h-full">
        ...
        <div className="bg-cyber-mid/90 backdrop-blur-xl h-full rounded-2xl p-4 sm:p-6 flex flex-col items-start min-h-[500px]">
            ...
            <div className="w-full flex justify-between items-center mb-6 border-b border-white/5 pb-4">
                <h2 className="text-xl sm:text-2xl font-bold flex items-center gap-2 text-white">
                    ...
                    {analysis?.isExplicit && (
                        <span className="ml-2 px-2 py-0.5 bg-red-500/20 text-red-400 border border-red-500/30 rounded text-[10px] font-bold uppercase tracking-widest">
                            Explicit
                        </span>
                    )}
                </h2>
            ...
            {isLoading && (
                <div className="flex-1 w-full flex flex-col items-center justify-center space-y-6">
                    <div className="relative w-24 h-24">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center text-[10px] font-mono text-emerald-400 animate-pulse text-center p-2">
                            NEURAL REASONING
                        </div>
                    </div>
                    <div className="font-mono text-[10px] text-emerald-500/80 text-center uppercase tracking-[0.2em]">Executing Gemini-3 Pro Reasoning...</div>
                </div>
            )}
            ...
            {analysis && !isLoading && (
                <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-5">
                    ...
                    <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                        <div className="text-[9px] text-gray-500 uppercase tracking-widest mb-1">Verified Credits</div>
                        <div className="text-sm font-bold text-white leading-tight">{analysis.artists}</div>
                    </div>
                    ...
                    <div className="bg-black/40 rounded-xl p-4 border border-white/5">
                        <div className="flex items-center justify-between mb-2">
                            <div className="text-[9px] text-emerald-500 uppercase tracking-widest font-bold font-mono">Algorithm Training</div>
                            <div className="flex gap-2">
                                <button 
                                  onClick={() => onFeedback('up')}
                                  title="Neural Sync (Correct)"
                                ...
                                <button 
                                  ...
                                  title="Neural Depth (Narrow down genre)"
                                ...
                                <button 
                                  ...
                                  title="Neural Reset (Incorrect classification)"
                        ...
                        <div className="text-[10px] text-gray-500 italic leading-relaxed">
                          {analysis.feedback === 'down' 
                            ? "Initiating contextual re-scan..." 
                            : analysis.feedback === 'refine_more'
                            ? "Identifying secondary sonic influences..."
                            : "Train the engine. Logic state: IDLE."}
                        </div>
                    </div>
                    ...
                    <div className="bg-black/20 rounded-xl p-4 border border-white/5 flex-1 overflow-y-auto max-h-[150px] custom-scrollbar">
                    ...
                    {analysis.isExplicit && analysis.profanityTimestamps && analysis.profanityTimestamps.length > 0 && (
                        <div className="bg-red-900/10 rounded-xl p-4 border border-red-500/20">
                            <div className="text-[10px] text-red-400 uppercase tracking-widest mb-2 font-mono font-bold flex items-center gap-2">
                            ...
                            <ul className="space-y-1">
                                {analysis.profanityTimestamps.map((timestamp, idx) => (
